...
| ------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Domain Testing (EP + BVA) | Every equivalence class has a corresponding `test()` block or is generated by a `for...of` loop array. Boundary values (on-point and off-point) have their own entries. No class is tested with multiple redundant values. |
...
**Data-driven pattern verification:** For each variant group annotated `[data-driven]` in `spec.md` Phase 2, verify it is implemented using a `for...of` loop generating a `test()` block (not manually duplicated). For each variant group annotated `[separate-test]`, verify each variant has its own manually written `test()` block (not generated by a loop). Flag a Coverage Gap finding if the annotation was not followed.
