...
When a flaky test fails only sometimes on an `expect()` matcher, the bare error message rarely explains _why_. Playwright 1.60 attaches `errorContext` to each `testInfo` error — including the aria snapshot of the element at the moment the matcher failed. Log it from `afterEach` so every flaky failure leaves a breadcrumb, even on the runs you can't reproduce locally.
