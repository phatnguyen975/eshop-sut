...
test("creates a new product via form", async ({ page }) => {
  ...
  await page
    ...
    .fill("Ergonomic wireless keyboard with backlit keys");
  ...
  await expect(
    page.getByRole("row", { name: /Wireless Keyboard/ }),
  ).toBeVisible();
  await expect(
    page.getByRole("row", { name: /Wireless Keyboard/ }),
  ).toContainText("$79.99");
...
test.describe("reading resources in a table", () => {
  test("displays resources in a table with correct columns", async ({
    page,
  }) => {
    await page.goto("/products");
  ...
  test("sorts table by column", async ({ page }) => {
    ...
    const numericDesc = pricesDesc.map((p) =>
...
test("updates an existing product", async ({ page }) => {
  ...
  await expect(
    page.getByRole("row", { name: /Wireless Keyboard Pro/ }),
  ).toBeVisible();
  await expect(
    page.getByRole("row", { name: /Wireless Keyboard Pro/ }),
  ).toContainText("$99.99");
...
test("edit form preserves data when navigating away and back", async ({
  page,
}) => {
  await page.goto("/products");
...
test("cancelling edit discards changes", async ({ page }) => {
  ...
  await expect(
    page.getByRole("row", { name: /Should Not Save/ }),
  ).not.toBeVisible();
});
```
...
test("create and verify via API", async ({ page, request }) => {
  ...
  const response = await request.get(
    "/api/products?search=API+Verified+Product",
  );
