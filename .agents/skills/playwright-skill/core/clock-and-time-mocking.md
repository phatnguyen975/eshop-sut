...
test("dashboard shows correct greeting based on time of day", async ({
  page,
}) => {
...
test("session timeout warning appears after 25 minutes", async ({ page }) => {
  ...
  await expect(
    page.getByRole("dialog", { name: "Session timeout" }),
  ).not.toBeVisible();
...
test("notification appears in real-time after scheduled trigger", async ({
  page,
}) => {
  ...
  await expect(page.getByTestId("notification-bell")).not.toHaveAttribute(
    "data-count",
  );
...
test("trial expiration banner shows at correct times", async ({ page }) => {
  ...
  await page.goto("/dashboard");
  await expect(page.getByTestId("trial-banner")).toContainText(
    "13 days remaining",
  );
  ...
  await expect(page.getByTestId("trial-banner")).toHaveCSS(
    "background-color",
    /rgb\(255/,
  ); // Red/warning
