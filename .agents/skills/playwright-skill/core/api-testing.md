...
**Avoid when**: The test specifically validates the creation flow through the UI. Seed everything _except_ what you are testing.
...
test("user sees their project on dashboard", async ({
  page,
  seedUser,
  seedProject,
}) => {
  ...
  await expect(
    page.getByRole("heading", { name: seedProject.name }),
  ).toBeVisible();
});
```
