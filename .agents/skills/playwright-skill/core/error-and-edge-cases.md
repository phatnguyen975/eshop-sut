...
test("intermittent connectivity — request fails then succeeds", async ({
  page,
}) => {
  let requestCount = 0;
...
**Use when**: A route handler sees a request that should _never_ happen (a call to a forbidden endpoint, a leaked third-party request, a missing mock), and you'd rather fail loudly at the source than chase a confusing downstream assertion.
...
test("empty state is shown when no items exist", async ({ page }) => {
  ...
  await expect(
    page.getByRole("link", { name: "Create your first task" }),
  ).toBeVisible();
...
test("handles zero, one, and many items (0-1-N pattern)", async ({ page }) => {
  ...
  await expect(
    page.getByRole("button", { name: /load more|show all/i }),
  ).toBeVisible();
});
```
...
test("browser forward returns to previous page", async ({ page }) => {
  ...
  await expect(
    page.getByRole("heading", { name: "Running Shoes" }),
  ).toBeVisible();
});
...
test("back button after form submission does not re-submit", async ({
  page,
}) => {
  await page.goto("/contact");
...
test("double-click submit does not create duplicate entries", async ({
  page,
}) => {
...
test("typing during navigation does not crash", async ({ page }) => {
  ...
  await page
    .getByLabel("Search")
    .pressSequentially("test query", { delay: 30 });
...
test("page works when third-party chat widget fails to load", async ({
  page,
}) => {
