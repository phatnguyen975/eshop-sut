...
test("renderer sends IPC message and gets response", async ({
  electronApp,
  window,
}) => {
...
test("main process handles IPC file-read request", async ({
  electronApp,
  window,
}) => {
...
test("open file dialog and load a document", async ({
  electronApp,
  window,
}) => {
...
test("save file dialog returns selected path", async ({
  electronApp,
  window,
}) => {
  await electronApp.evaluate(async ({ dialog }) => {
  ...
  await expect(
    window.getByText("Saved to /tmp/exported-report.pdf"),
  ).toBeVisible();
});
...
test('clicking tray "Show" menu item opens the window', async ({
  electronApp,
}) => {
...
test("open and interact with preferences window", async ({
  electronApp,
  window,
}) => {
