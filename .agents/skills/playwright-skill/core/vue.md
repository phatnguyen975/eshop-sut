...
test("respects max prop", async ({ mount }) => {
  ...
  await expect(
    component.getByRole("button", { name: "Increment" }),
  ).toBeDisabled();
});
...
test.describe("Vue Router navigation", () => {
  ...
  test("query parameters update reactive computed properties", async ({
    page,
  }) => {
...
test.describe("Teleport components", () => {
  test("modal teleported to body is visible and interactive", async ({
    page,
  }) => {
    await page.goto("/products");
  ...
  test("teleported notification appears above all content", async ({
    page,
  }) => {
    await page.goto("/settings");
...
test.describe("transitions", () => {
  ...
  test("deleted items animate out and are removed from DOM", async ({
    page,
  }) => {
    await page.goto("/todo");
  ...
  test("disable animations for faster, more reliable tests", async ({
    page,
  }) => {
...
test.describe("composition API patterns", () => {
  ...
  test("composable hook (useSearch) provides debounced search", async ({
    page,
  }) => {
    await page.goto("/products");
  ...
  test("provide/inject context works across component tree", async ({
    page,
  }) => {
    await page.goto("/dashboard");
