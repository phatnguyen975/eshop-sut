...
test.describe("Pages Router with SSR", () => {
  test("page with getServerSideProps renders fetched data", async ({
    page,
  }) => {
  ...
  test("page with getStaticProps shows pre-rendered content", async ({
    page,
  }) => {
  ...
  test("client-side navigation with next/link preserves SPA behavior", async ({
    page,
  }) => {
    await page.goto("/blog");
    ...
    await expect(
      page.getByRole("heading", { name: "My First Post", level: 1 }),
    ).toBeVisible();
  });
});
```
