...
When two controls share the same accessible name but differ by their accessible _description_ (`aria-describedby` / `aria-description`), Playwright 1.60's `description` option on `getByRole()` (and the other `getBy*` roles) targets the right one without falling back to CSS.
...
test("legacy app with no semantic markup", async ({ page }) => {
  ...
  await page
    .locator('table.report-grid td:has-text("Overdue")')
    .first()
    .click();

  // XPath — only when CSS cannot express the query (e.g., text + ancestor traversal)
  await page
    .locator('xpath=//td[contains(text(),"Overdue")]/ancestor::tr//button')
    .click();
