...
test("reorder persists after page reload", async ({ page }) => {
  ...
  await page.waitForResponse(
    (response) =>
      response.url().includes("/api/tasks/reorder") &&
      response.status() === 200,
  );
...
test("reorders with precise mouse movements for libraries that need it", async ({
  page,
}) => {
  await page.goto("/tasks");
...
test("moves a card from Todo to In Progress", async ({ page }) => {
  ...
  await expect(todoColumn.getByRole("article")).toHaveCount(
    todoCountBefore - 1,
  );
  await expect(inProgressColumn.getByRole("article")).toHaveCount(
  ...
  await expect(
    inProgressColumn.getByText(`(${inProgressCountBefore + 1})`),
  ).toBeVisible();
});
...
test("uploads a file via the underlying input (Playwright < 1.60)", async ({
  page,
}) => {
  await page.goto("/upload");
  ...
  await fileInput.setInputFiles(
    path.resolve(__dirname, "../fixtures/sample-document.pdf"),
  );
...
test("simulates drag-over visual feedback via JavaScript dispatch", async ({
  page,
}) => {
  await page.goto("/upload");
...
test("drag preview shows item count for multi-select drag", async ({
  page,
}) => {
  await page.goto("/board");
...
test("drags an element to a specific coordinate on a canvas", async ({
  page,
}) => {
