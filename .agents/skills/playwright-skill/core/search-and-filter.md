...
test("searches for a product and displays matching results", async ({
  page,
}) => {
...
test("shows loading indicator during debounced search", async ({ page }) => {
  ...
  await expect(
    page.getByRole("progressbar").or(page.getByText(/loading|searching/i)),
  ).toBeVisible();
...
test("filters products by a single category", async ({ page }) => {
  ...
  await expect(
    page
      .getByText(/filtered by.*electronics/i)
      .or(
        page
...
test("filters by multiple categories simultaneously", async ({ page }) => {
  ...
  await expect(
    page.getByRole("button", { name: /Electronics.*×|Remove Electronics/i }),
  ).toBeVisible();
  await expect(
    page.getByRole("button", { name: /Clothing.*×|Remove Clothing/i }),
  ).toBeVisible();
});
...
test("removes one filter from a multi-select filter", async ({ page }) => {
  ...
  await page
    .getByRole("button", { name: /Clothing.*×|Remove Clothing/i })
    .click();
  await page.waitForResponse("**/api/products?*");
...
test("shows error for invalid date range", async ({ page }) => {
  ...
  await expect(
    page.getByText(/end date.*before.*start date|invalid.*range/i),
  ).toBeVisible();
});
...
test("clear all button only appears when filters are active", async ({
  ...
  await expect(
    page.getByRole("button", { name: /clear all|reset filters/i }),
  ).not.toBeVisible();
...
test("selects a suggestion with click", async ({ page }) => {
  ...
  await expect(
    page.getByRole("row", { name: /Wireless Keyboard/i }),
  ).toBeVisible();
});
...
test('shows "no results" state for unmatched search', async ({ page }) => {
  ...
  await expect(
    page.getByText(/no results|no products found|nothing found/i),
  ).toBeVisible();
  ...
  await expect(
    page
      .getByRole("button", { name: /clear search|show all/i })
      .or(page.getByRole("link", { name: /clear search|show all/i })),
  ).toBeVisible();
});

test("no results state with active filters offers to clear them", async ({
  page,
}) => {
  await page.goto("/products");
  ...
  await expect(
    page.getByRole("button", { name: /clear filters|reset/i }),
  ).toBeVisible();
...
test("no results state illustration is displayed", async ({ page }) => {
  ...
  await expect(
    page
      .getByRole("img", { name: /no results|empty/i })
      .or(page.locator('[data-testid="empty-state-illustration"]')),
  ).toBeVisible();
});
```
...
test('shows "no results" state for unmatched search', async ({ page }) => {
  ...
  await expect(
    page.getByText(/try different|broaden your search/i),
  ).toBeVisible();
});
...
test("pagination resets to page 1 when a filter is applied", async ({
  page,
}) => {
  await page.goto("/products");
...
test("filters persist across pagination", async ({ page }) => {
  ...
  await expect(
    page.getByRole("complementary").getByLabel("Electronics"),
  ).toBeChecked();
...
test("search results are shareable via URL", async ({ page, context }) => {
  ...
  await expect(newPage.getByRole("searchbox", { name: /search/i })).toHaveValue(
    "keyboard",
  );
  await expect(
    newPage.getByRole("complementary").getByLabel("Electronics"),
  ).toBeChecked();

  const rows = newPage
    .getByRole("row")
    .filter({ hasNot: newPage.getByRole("columnheader") });
  await expect(rows.first()).toBeVisible();
...
test("filter options show result counts", async ({ page }) => {
  ...
  const clothingText = await sidebar
    .getByText(/Clothing \(\d+\)/)
    .textContent();
