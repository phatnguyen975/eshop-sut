...
test("wait for server acknowledgment over WebSocket", async ({ page }) => {
  ...
  await expect(page.getByTestId("message-status").last()).toHaveText(
    "Delivered",
  );
});
```
...
test("inject a fake high-priority message into real stream", async ({
  page,
}) => {
...
test("polling updates dashboard data every interval", async ({ page }) => {
  ...
  await expect(page.getByTestId("active-users")).toHaveText("142", {
    timeout: 15000,
  });
