...
The CLI is not a lesser Trace Viewer — it is the _agent-native_ interface to the same data.
...
3. **`action <id>` for the error, `snapshot <id>` for the page state** — the error says _what_ failed; the snapshot says _why the page wasn't ready_.
