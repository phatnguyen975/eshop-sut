...
test("uploads a file created in-memory (no fixture file needed)", async ({
  page,
}) => {
  await page.goto("/documents");
...
test("uploads a file via a native drop (Playwright 1.60+)", async ({
  page,
}) => {
  await page.goto("/documents");
  ...
  await dropZone.drop({
    files: path.resolve(__dirname, "../fixtures/report.pdf"),
  });
...
test("downloads a file via the stream (no disk save needed)", async ({
  page,
}) => {
  await page.goto("/documents");
...
test("download has correct MIME type via response headers", async ({
  page,
}) => {
  await page.goto("/reports");
...
test("cancels an in-progress upload", async ({ page }) => {
  ...
  await expect(
    page.getByRole("link", { name: "large-file.bin" }),
  ).not.toBeVisible();
});
...
test("rejects disallowed file types with clear error message", async ({
  page,
}) => {
  await page.goto("/documents");
...
test("shows image preview after selecting a file", async ({ page }) => {
  ...
  await fileInput.setInputFiles(
    path.resolve(__dirname, "../fixtures/avatar.jpg"),
  );
...
```typescript
test("downloads a file that requires authentication", async ({
  page,
  request,
}) => {
