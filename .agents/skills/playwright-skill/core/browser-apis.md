...
test("notification permission granted shows notification UI", async ({
  browser,
}) => {
  const context = await browser.newContext({
...
test("notification permission denied shows upgrade prompt", async ({
  browser,
}) => {
...
test("paste from clipboard into editor", async ({ browser }) => {
  ...
  await page.evaluate(() =>
    navigator.clipboard.writeText("Pasted content from clipboard"),
  );
...
test("sessionStorage survives navigations within the session", async ({
  page,
}) => {
...
test("offline-first app stores data in IndexedDB", async ({ page }) => {
  ...
  await page
    ...
    .fill("This is stored in IndexedDB");
...
test("app triggers a browser notification on new message", async ({
  browser,
}) => {
  const context = await browser.newContext({
    permissions: ["notifications"],
