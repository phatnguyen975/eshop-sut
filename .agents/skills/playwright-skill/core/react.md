...
test.describe("useDebounce hook (tested through SearchInput)", () => {
  test("debounced search triggers after user stops typing", async ({
    page,
  }) => {
...
test.describe("auth context", () => {
  test("login updates the UI across all components consuming auth context", async ({
    page,
  }) => {
    await page.goto("/");
...
test.describe("React Router navigation", () => {
  test("client-side navigation does not cause full page reload", async ({
    page,
  }) => {
    await page.goto("/");
  ...
  test("browser back button works with client-side routing", async ({
    page,
  }) => {
    await page.goto("/");
    await page.getByRole("link", { name: "Products" }).click();
...
test.describe("registration form (react-hook-form)", () => {
  ...
  test("password requirements update in real time", async ({ page }) => {
    ...
    await expect(page.getByText("Contains uppercase")).toHaveClass(
      /text-green/,
    );
    await expect(page.getByText("Contains number")).toHaveClass(/text-green/);
    await expect(page.getByText("Contains special character")).toHaveClass(
      /text-green/,
    );
  });
  ...
  test("disables submit button while form is submitting", async ({ page }) => {
    ...
    await expect(
      page.getByRole("button", { name: /Creating|Loading|Submitting/ }),
    ).toBeDisabled();
  });
});
```
...
test.describe("portals", () => {
  ...
  test("dropdown menu rendered via portal positions correctly", async ({
    page,
  }) => {
    await page.goto("/dashboard");
...
test.describe("error boundaries", () => {
  test("error boundary shows fallback UI when child crashes", async ({
    page,
  }) => {
  ...
  test("retry button in error boundary recovers the component", async ({
    page,
  }) => {
    let requestCount = 0;
    await page.route("**/api/dashboard/widgets", (route) => {
...
```typescript
test("no state-update-on-unmounted warnings during navigation", async ({
  page,
}) => {
