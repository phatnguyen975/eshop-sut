...
test("extension popup displays saved bookmarks", async ({
  page,
  extensionId,
}) => {
...
test("content script injects price comparison widget", async ({ context }) => {
  ...
  await expect(page.getByTestId("price-compare-widget")).toBeVisible({
    timeout: 10000,
  });
...
test("background worker processes messages correctly", async ({
  context,
  extensionId,
}) => {
  const page = await context.newPage();
...
test("service worker handles extension storage", async ({
  context,
  extensionId,
}) => {
  const page = await context.newPage();
...
test("options page saves preferences", async ({ page, extensionId }) => {
  ...
  await expect(
    page.getByRole("checkbox", { name: "Auto-update" }),
  ).toBeChecked();
});
```
