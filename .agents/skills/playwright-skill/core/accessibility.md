...
test("checkout dialog exposes the expected accessibility structure", async ({
  page,
}) => {
...
test.describe("accessibility", () => {
  ...
  test("dashboard has no accessibility violations after login", async ({
    page,
  }) => {
  ...
  test("report violations with helpful details on failure", async ({
    page,
  }) => {
...
test.describe("WCAG compliance levels", () => {
  ...
  test("meets WCAG AAA (strict — use for government or healthcare)", async ({
    page,
  }) => {
    await page.goto("/");
...
test.describe("accessibility with known exceptions", () => {
  test("no new violations (excluding tracked known issues)", async ({
    page,
  }) => {
    await page.goto("/dashboard");
  ...
  test("verify known issues still exist (remove when fixed)", async ({
    page,
  }) => {
    await page.goto("/dashboard");
...
test.describe("screen reader semantics", () => {
  ...
  test("expandable sections announce their state", async ({ page }) => {
    ...
    const panel = page.locator(
      `#${await faqButton.getAttribute("aria-controls")}`,
    );
...
test.describe("color contrast", () => {
  ...
  test("high contrast mode meets AAA contrast requirements", async ({
    page,
  }) => {
...
test.describe("focus trap", () => {
  ...
  test("dropdown menu traps focus and returns it on close", async ({
    page,
  }) => {
    await page.goto("/dashboard");
  ...
  test("background content is inert when modal is open", async ({ page }) => {
    ...
    expect(
      isHidden,
      "Background content must be hidden from assistive technology",
    ).toBe(true);
  });
});
```
...
test.describe("accessible forms", () => {
  ...
  test("error messages are linked to their fields via aria-describedby", async ({
    page,
  }) => {
    await page.goto("/register");
  ...
  test("form error summary is announced and links to fields", async ({
    page,
  }) => {
    await page.goto("/register");
    await page.getByRole("button", { name: "Create account" }).click();
  ...
  test("autocomplete attributes are set for common fields", async ({
    page,
  }) => {
    await page.goto("/checkout");
...
test("dashboard has no violations after data loads", async ({
  page,
  makeAxeBuilder,
}) => {
  await page.goto("/dashboard");
