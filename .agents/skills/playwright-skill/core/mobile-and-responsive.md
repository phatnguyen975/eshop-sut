...
test("mobile user can navigate via hamburger menu", async ({
  page,
  isMobile,
}) => {
  await page.goto("/");
...
for (const bp of breakpoints) {
  test(`layout adapts correctly at ${bp.name} (${bp.width}px)`, async ({
    page,
  }) => {
...
import {
  ...
  expect,
  describeBreakpoints,
} from "./fixtures/responsive.fixture";
...
const {
  ...
  expect,
  describeBreakpoints,
} = require("./fixtures/responsive.fixture");
...
test("offline fallback page shows when uncached route is accessed", async ({
  page,
}) => {
  await page.goto("/");
...
test("push notification permission request on mobile", async ({
  page,
  context,
}) => {
