...
test("use Frame API for URL checks and evaluate", async ({ page }) => {
  ...
  await page
    .frameLocator('iframe[name="analytics"]')
...
test("access closed shadow DOM by forcing open mode", async ({ page }) => {
  ...
  await page
    .locator("closed-component")
    .getByRole("button", { name: "Action" })
    .click();
  await expect(
    page.locator("closed-component").getByText("Done"),
  ).toBeVisible();
});
```
...
test("slotted content is visible through web component", async ({ page }) => {
  ...
  await expect(
    card.getByRole("heading", { name: "Product Title" }),
  ).toBeVisible();
