...
await page
  .getByLabel("Upload")
  .setInputFiles(["fixtures/a.png", "fixtures/b.png"]);
...
test("upload multiple attachments", async ({ page }) => {
  ...
  await expect(page.getByTestId("file-list").getByRole("listitem")).toHaveCount(
    3,
  );
...
downloadTest(
  "save downloads to organized directories",
  async ({ page, downloadDir }) => {
...
test("rejects unsupported file types", async ({ page }) => {
  ...
  await expect(
    page.getByText("Only JPG, PNG, and GIF files are allowed"),
  ).toBeVisible();
