...
test("critical assets are cached by the service worker", async ({
  swContext,
}) => {
  const page = await swContext.newPage();
  await page.goto("/");
...
test("cache is invalidated after service worker update", async ({
  swContext,
}) => {
  const page = await swContext.newPage();
  await page.goto("/");
...
test("app captures install prompt and shows install button", async ({
  swContext,
}) => {
  const page = await swContext.newPage();
...
test("background sync retries failed requests when online", async ({
  swContext,
}) => {
  const page = await swContext.newPage();
