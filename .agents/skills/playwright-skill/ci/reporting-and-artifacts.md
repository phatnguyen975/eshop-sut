...
test("payment integration", async ({ context, page }, testInfo) => {
  ...
  await testInfo.attach("payment-network", {
    path: harPath,
...
The 1.61 modes mirror the `trace` modes, so video and trace policies can finally match. `'retain-on-failure-and-retries'` is the best default for flaky-test hunting: you get video of both the original failure _and_ the retry, so you can compare the two runs.
